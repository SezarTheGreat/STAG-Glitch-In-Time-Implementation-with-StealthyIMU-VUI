            ...
            if stage == sb.Stage.TEST:
                ...
                with jsonlines.open(
                    ...
                    for i in range(len(predicted_semantics)):
                        ...
                        pred_dict["file"] = getattr(self, "id_to_file", {}).get(ids[i], ids[i])
...
def dataio_prepare(hparams):
    ...
    def audio_pipeline(wav):
        ...
        # Check if dynamic stacking upscaling is enabled
        if hparams.get("use_stacking_upscaler", False):
            try:
                global stacking_upscaler
                if 'stacking_upscaler' not in globals():
                ...
                # Dynamic reconstruction from raw .acc and .gyro files
                ...
                signal = torch.from_numpy(reconstructed_z).float().to('cpu')
                return signal
            except Exception as e:
                # print(f"[WARN] Stacking upscaling failed: {e}. Falling back to precomputed accnpy.")
                pass

        # Fallback to precomputed accnpy
